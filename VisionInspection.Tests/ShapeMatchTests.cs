...
public class ShapeMatchTests
{
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void RepeatedFind_IsDeterministic(bool fastMode)
    {
        var template = BuildTemplateFromStandard();
        using var image = DrawShape(512, 384, 256, 192, 7);
        var matcher = new ShapeMatcher(template, 0.5, -20, 40, 1, 0.3, 30, 1.0, true, fastMode);

        var first = Assert.Single(matcher.Find(image));
        for (var i = 0; i < 4; i++)
        {
            var current = Assert.Single(matcher.Find(image));
            Assert.Equal(first.X, current.X, 10);
            Assert.Equal(first.Y, current.Y, 10);
            Assert.Equal(first.Angle, current.Angle, 10);
            Assert.Equal(first.Score, current.Score, 10);
        }
    }
    ...
    [Fact]
    public void Builder_DistributesPointsAcrossTemplateArea()
    {
        using var patch = new Mat(140, 180, MatType.CV_8UC1, Scalar.All(30));
        Cv2.Rectangle(patch, new Rect(8, 8, 164, 124), Scalar.All(220), 2);
        Cv2.Line(patch, new Point(12, 70), new Point(168, 70), Scalar.All(220), 2);

        var template = ShapeTemplateBuilder.Build(patch, 90, 70, 1.0, 10, 1, 64);
        var cells = template.LevelPoints[0]
            .Select(p => (X: Math.Clamp((int)Math.Floor((p.X + 90) / 180 * 4), 0, 3),
                Y: Math.Clamp((int)Math.Floor((p.Y + 70) / 140 * 4), 0, 3)))
            .Distinct()
            .Count();

        Assert.True(cells >= 4, $"空间覆盖网格过少: {cells}");
    }
    ...
    [Fact]
    public void Matcher_RejectsTemplateOutsideSearchImage()
    {
        var rotated = ShapeMatcher.BuildRotated(
            [-20, 20], [0, 0], [1, 1], [0, 0], 0);

        Assert.True(ShapeMatcher.IsTemplateInsideSearchImage(rotated, 50, 20, 25, 10));
        Assert.False(ShapeMatcher.IsTemplateInsideSearchImage(rotated, 50, 20, 35, 10));
    }
