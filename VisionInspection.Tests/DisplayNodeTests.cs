...
public class AutoDisplayTests
{
    ...
    [Fact]
    public void OverlayDisplay_UsesExplicitNodesAndPreservesAnnotations()
    {
        NodeFactory.Register("FakeOverlaySource", (n, _) => new FakeNode(n, _ => new NodeResult
        {
            Decision = "NG",
            Values = { ["score"] = "0.91", ["decision"] = "NG" },
            OutputImage = new Mat(12, 16, MatType.CV_8UC3, Scalar.All(80)),
            Annotations =
            {
                new NodeShape { Box = new OpenCvSharp.Rect(2, 3, 4, 5), Label = "缺陷", Kind = NodeShapeKind.Defect },
            },
        }));
        var recipe = new Recipe
        {
            Nodes =
            [
                new RecipeNode { Name = "01 检测", Type = "FakeOverlaySource", Enabled = true },
                new RecipeNode
                {
                    Name = "02 叠加显示", Type = "OverlayDisplay", Enabled = true,
        ...
        using var pipeline = new Pipeline(recipe);
        using var input = new Mat(12, 16, MatType.CV_8UC3, Scalar.All(10));
        var result = pipeline.Run(input, "t");
        try
        {
            Assert.Equal("02 叠加显示", result.DisplayNodeName);
            Assert.Equal(2, result.NodeAnnotations["02 叠加显示"].Count);
            Assert.Contains(result.NodeAnnotations["02 叠加显示"], s => s.Label?.Contains("01 检测", StringComparison.Ordinal) == true);
            Assert.Equal("1", result.NodeValues["02 叠加显示"]["overlay_count"]);
            Assert.Equal("OK", result.NodeValues["02 叠加显示"]["decision"]);
        }
        finally
        {
            foreach (var image in result.NodeImages.Values) image.Dispose();
        }
    }

    [Fact]
    public void OverlayDisplay_WithEmptySelectionDoesNotAddAnnotations()
    {
        var node = new OverlayDisplayNode("叠加")
        {
            Log = _ => { },
        };
        using var input = new Mat(4, 4, MatType.CV_8UC3, Scalar.All(10));
        var result = node.Run(input, new PipelineRunContext(input));
        Assert.Empty(result.Annotations);
        Assert.Equal("0", result.Values["overlay_count"]);
